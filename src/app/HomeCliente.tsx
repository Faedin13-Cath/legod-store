...
/**
 * La figura del hero. Va escrita aquí y no leída del catálogo a propósito: es
 * la imagen más grande de la página y la que marca la velocidad de carga. Si
 * dependiera de la petición a Shopify, el hero saldría vacío el primer segundo.
 */
const HERO = {
              ...
              <div className="anim-scale-in d-2" style={{ position:'absolute', inset:0, borderRadius:32, overflow:'hidden', boxShadow:'0 48px 96px -24px rgba(0,0,0,0.7)', background:'#fff' }}>
                <div className="anim-float" style={{ position:'absolute', inset:0 }}>
                  <div style={{ position:'absolute', inset:0, transform:'scale(0.82)', transformOrigin:'center' }}>
                    <Image src={shopifyImg(heroFoto, 900)!} alt={HERO.alt} fill style={{ objectFit:'contain', objectPosition:'center' }} sizes="45vw" priority />
                  </div>
                </div>
          ...
          <Reveal delay={0} animation="scale-in">
            <Link href="/tienda/winter-soldier-dark-brown-hair-black-and-light-nougat-hands" className="promo-tile" style={{ borderRadius:28, overflow:'hidden', position:'relative', display:'flex', flexDirection:'column', justifyContent:'space-between', minHeight:200, textDecoration:'none', background:'linear-gradient(160deg,#1A1266,#2D1869)', padding:24 }}>
              <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'flex-end', paddingRight:16, overflow:'hidden' }}>
                <Image src="https://cdn.shopify.com/s/files/1/1012/4443/6789/files/sh1132.png?width=280" alt="Bucky (Winter Soldier)" width={140} height={180} style={{ objectFit:'contain', opacity:0.95 }} />
              </div>
          ...
          {[
            ...
            <Reveal key={b.icon} delay={i * 80} animation="fade-up" style={{ height:'100%' }}>
              <div style={{ height:'100%', background:'var(--paper)', border:'1px solid var(--line)', borderRadius:20, padding:'24px 20px', display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <h3 style={{ margin:0, fontSize:15, fontWeight:600, color:'var(--ink)' }}>{b.title}</h3>
